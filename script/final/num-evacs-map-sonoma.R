...
ct$n_order[94]<-17
Sonoma$GEOID <- substr(Sonoma$GEOID, 2, 100)

#was missing this step
Sonoma <- left_join(Sonoma, ct)




Sonoma %>%
  ggplot(aes(fill = Sonoma$n_order)) + #this was previously ct$no_order but it's not in the same order as Sonoma so it was plotting the wrong colors
  geom_sf(color = NA) + 
  coord_sf(crs = 26911) + 
