...
# Read in Cleaned Evacuation Data for Sonoma County ##########################
evac1 <- read.csv("data/clean/Sonoma Census Tract Evacuation Order Mapping.csv") %>%
  dplyr::select(GEOID, NAME, starts_with("evacuation")) %>%
  mutate(GEOID = as.character(GEOID))

evac1$n_order <- apply(evac1[,paste("evacuation",1:29, sep = "")], 1, function(x){sum(!is.na(x))})

#############################################################################
acs$evac <-evac1$n_order
...
#png(filename = "Race.png", width=800, height=400)
#Evacuated Population Matrix 
par(mfrow =c (1,2))
...
#dev.off()
#Not Evacuated Population Matrix 
...
png(filename = "RacePie.png", width=800, height=400)
race_dist <- as.data.frame(totaloutput) # <-- this would be to get the pie chart for the total population
...
pie(slices, labels = lbls, main="Racial Breakdown of Sonoma County", col = c("grey20", "grey40", "grey60", "grey80", "grey100"))
dev.off()

png(filename = "QuintilePie1.png", width=800, height=400)
#pie chart of county income distribution
inc_dist <- as.data.frame(output1) #Output1 = not evacuated; Output0= Evacuated
